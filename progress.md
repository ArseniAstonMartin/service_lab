...
## 2026-09-26 — TASK-037: Confirm-compatibility dialog in the review queue
- `ReviewQueueCard` is now a client component whose click opens `ConfirmCompatibilityDialog` (enlarged sticker photo, part number, vehicle, module) instead of navigating away; a plain "View full order details →" link inside it still reaches TASK-031's detail page. Checkboxes (hand-rolled, no shadcn Checkbox installed) list the order's category's services; a shadcn `Select` — options limited to the checked ones, reset if its current value gets unchecked — picks `selectedServiceId`. Submit calls `confirmCompatibility`, shows inline+toast errors, and on success closes and calls `router.refresh()` (re-runs the queue list and the layout's pending-count badge; `confirmCompatibility` already `revalidatePath`s both server-side).
- `/admin/review-queue`'s page now also queries all `Service`s once and groups them by `categoryId` client-side, so each card gets only its own category's services without an N-query fan-out.
...
## 2026-09-26 — TASK-039: Excel/CSV import Server Actions (preview + apply)
- `lib/domain/import.ts` (new, pure): `validateImportRow()` matches Make/Model/Year/Category/Part Number columns case-insensitively/trimmed against the live catalog, resolving each row or collecting reasons (missing make/model, invalid/out-of-range year, empty part number, unknown category, unknown service column — the last only fires when that row actually has a '1'/'x' value in an unrecognized column; a blank cell under an unknown column is not an error). `collectUnknownColumns()` reports file-level ignored-column warnings separately. `lib/actions/import.ts`: `previewImport(file)` (`requireAdmin`) dispatches `.csv`→papaparse / `.xlsx`/`.xls`→SheetJS `xlsx`, both normalized to the same header→string-cell row shape, validates every row against a fresh category/service read, and writes nothing. `applyImport(rows)` re-resolves every row's category/services against a fresh DB read (never trusting `previewImport`'s resolution), then in batches of 25 rows per `$transaction`: upserts `Vehicle`, upserts `CompatibilityEntry` (`source: import`), replaces its `CompatibilityService` links, and counts added/updated/skipped with reasons.
- Bumped `next.config.mjs`'s `experimental.serverActions.bodySizeLimit` to `10mb` (default 1MB is too small for a spreadsheet passed straight through a Server Action). Installed `xlsx`, `papaparse`, `@types/papaparse`.
- Verified with `tsc --noEmit` and a full `next build`; additionally ran the pure `lib/domain/import.ts` functions directly under `tsx` against 18 hand-built cases (valid row, each rejection reason, the blank-vs-filled unknown-column distinction, dedup of unknown columns, year bounds) and separately smoke-tested papaparse/SheetJS round-tripping a real in-memory CSV and .xlsx — all passed. No UI exists yet to click through (that's TASK-040); confirmed `applyImport`'s upsert/unique-key names (`make_model_year`, `vehicleId_categoryId_partNumber`) and `EntrySource` values against `schema.prisma` directly.
- **Next up:** TASK-040 (the import screen: file picker + preview table + Apply button) is the direct, now-unblocked follow-on that makes this action reachable. TASK-033/034 remain the unblocked medium picks.

## 2026-09-26 — TASK-040: Excel/CSV import screen with preview
- `/admin/compatibility/import` (new): file picker → `previewImport()` → a preview showing valid-row and error-row (with reasons) tables plus unknown-column warnings → an "Apply N rows" button (disabled with zero valid rows) → `applyImport()` result counts and any post-apply skipped rows. `CompatibilityImport` (new client component) owns all of this state locally. Added a revalidatePath("/admin/compatibility") call inside `applyImport` itself (was missing from TASK-039) so the acceptance criteria's "revalidates the compatibility list" holds without relying on the client. Linked from `/admin/compatibility` via a new "Import Excel/CSV" button.
- Verified with `tsc --noEmit` and a full `next build` (route compiles, correctly dynamic, 4.67 kB); confirmed in-browser that an unauthenticated visit to `/admin/compatibility/import` redirects to `/admin/login` like every other admin page — no admin credentials available in this session to click through the actual preview/apply flow.
- **Next up:** TASK-033 (label upload) and TASK-034 (customer history) are the remaining unblocked medium picks; TASK-041 (Resend setup) and TASK-044 (pricing admin) are the next unblocked high-priority picks.

## 2026-09-26 — TASK-033: Admin return label upload, replace and remove
- `lib/actions/shipping-label.ts` (new): `setShippingLabel({orderId, blobUrl})` and `removeShippingLabel({orderId})` (`requireAdmin`) are now the only writers of `Order.shippingLabelUrl`. `setShippingLabel` validates the URL is a real Vercel Blob URL under the `labels/` prefix (`isVercelBlobUrl` + pathname check), commits the new URL first, then deletes the old blob with `del()` best-effort (swallowed on failure — never blocks the admin action). `removeShippingLabel` clears the field then deletes its blob the same way. Both revalidate the order detail page and the order's own `/track/[token]` page.
- `ShippingLabelUpload` (new client component) on the order detail page: uploads directly to Blob via the existing "label"-purpose token route (TASK-014, admin-only), then calls `setShippingLabel`; shows a "View current label" link + Replace/Remove once one exists. Not built on the wizard's `<FileUpload>` since that always renders an `<img>` preview, which breaks for PDFs.
- Verified with `tsc --noEmit` and a full `next build` (`/admin/orders/[id]` compiles, correctly dynamic). The label then surfaces on `/track/[token]` via TASK-029's existing conditional download button (already reads `shippingLabelUrl`, no changes needed there). No admin login/dev-server available in this session for an in-browser click-through, same limitation as prior UI tasks.
- **Next up:** TASK-034 (customer history) is the last unblocked medium pick; TASK-041 (Resend) and TASK-044 (pricing admin) are the next unblocked high-priority picks.

## 2026-09-26 — TASK-034: Admin customer history by email
- The order detail page's customer email now links to `/admin/orders?email=…` (was `?q=…`, a "contains" search). `/admin/orders` gained an `email` searchParam that filters `customerEmail` with an exact, case-insensitive match, independent of `q`/status/dates, plus a summary bar showing that customer's total order count and total paid (sum of `totalAmountCents` for orders in `payment_received` or later — `isPostPaymentStatus`) computed unfiltered by any other param, so it always reflects the customer's whole history. `OrdersFilterBar` shows it as a removable "Customer: …" chip.
- Verified with `tsc --noEmit` and a full `next build`; read-only SQL against the live project confirmed the `OrderStatus` enum strings used in the paid-status filter match the DB exactly (both real orders are `pending_review`, so the paid-summary path itself has no non-zero real data yet, but the query runs clean).
- **Next up:** all `medium`-priority tasks are now done. TASK-041 (Resend emails) and TASK-044 (pricing admin) are the next unblocked `high`-priority picks.

## 2026-09-26 — TASK-041: Resend setup, email templates and the send helper
- `lib/email/send.ts`: `sendOrderEmail(orderId, type)` renders one of the 5 PRD-section-9 emails (`order_submitted`, `payment_link`, `module_received`, `ready_shipped_back`, `admin_new_order`) with `react-dom/server`'s `renderToStaticMarkup`, sends it via a lazily-constructed Resend client (`lib/email/resend-client.ts`, same lazy-Proxy pattern as `lib/stripe.ts`), and records the outcome in `EmailLog`. A `findUnique` on the `(orderId, emailType)` key skips already-sent emails without calling Resend; the DB's own unique constraint is the backstop for the rare concurrent-call race. Never throws — every failure (missing order field, Resend API error, invalid id) is caught, logged to `EmailLog` + console, and returned as `{sent:false, error}`, so a caller (TASK-042/043) can call this after its own commit without risking that commit's success.
- `lib/email/templates/*.tsx` (5 new, plus a shared `layout.tsx`): plain TSX (no `@react-email/components` dependency), inline-styled tables since email clients strip `<style>`/classes.
